...
## Focused iteration 36134100111 / 71aa2ad — final failure evidence

Job 108067913581 complete/failure; raw and decoded logs saved, command receipts extracted. No artifact uploaded (remote artifact list empty). Targeted core exit 0, 247/247 (20+24+16+14+173), Router core bridge exit 0 8/8. Native exit 1: 124 passed / 1 failed of 125 in 7 files. Sole native failure is cleanup-test assertion that closeAndWait must be called. Core types exit 1: new TS2345 at owner-hook integration line64, optional invocation order passed to toBeLessThan. Extension types exit 0. Check step exit143 after runner shutdown; no command receipt or internal output because wrapper buffered to local file. Its result is interrupted/UNKNOWN, not a proven source failure. Summary/capture/upload skipped on runner shutdown. No full run or packaging.

Native failure source audit: createStartedThreadHarness installs an injected client factory, not a registered shared-client pool entry. run-attempt-resources.ts only calls closeAndWait when clearSharedCodexAppServerClientIfCurrentAndUnclaimed reports retired.closed; missing shared entry yields false and one-shot cleanup throws 'Codex one-shot client cleanup could not be confirmed'. finishHookBlock deliberately catches/logs that cleanup failure via runCleanupStep and preserves the policy terminal. Test already observed hook terminal/no submission correctly but spied on the wrong unreachable method. Replace with assertions for the actual cleanup warning and unsubscribe plus unchanged policy terminal/no turn/start, without weakening authority gates.

Iteration 2 plan: guard both invocation-order numbers explicitly; validate actual one-shot cleanup failure path; stream check logs and upload per-stage focused receipts to survive runner interruption. Keep every original full gate required. Retest targeted/native/core types/extension types/check; no build/ingress/lifecycle/contracts until focused all pass.
