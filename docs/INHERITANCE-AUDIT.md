...
Native adapter gap confirmed upstream: run-attempt-context.ts builds a subset without owner/guards, and no before_agent_run invocation existed. The revised candidate forwards full facts and adds the gate before native turn/start, including retries. Exact official Codex dependency 0.153.4 was inspected at peeled commit 3d2ee51ca2d5db578f328aa75e20aa22c0197c9a. Native UserPromptSubmit is a distinct boundary, not evidence that OpenClaw hooks already ran. Added adapter regression tests and an actual native-adapter/Router bridge await isolated CI. Fresh source review found no remaining blocking issue; no runtime PASS is inferred.
