...
import { classifySeniorModelFailure, clearStopState, readStopState, writeStopState } from "../pilot.js";
...
test("HIGH + genuine Astra unavailable is explicit STOP evidence", () => {
  assert.deepEqual(classifySeniorModelFailure(modelError({ errorCategory: "provider_unavailable" }), ASTRA), {
    unavailable: true,
...
test("HIGH + user/caller abort is not unavailable", () => {
  assert.equal(classifySeniorModelFailure(modelError({ failureKind: "aborted" }), ASTRA).unavailable, false);
});

test("HIGH + gateway/recovery interruption is not unavailable", () => {
  assert.equal(classifySeniorModelFailure(modelError({ failureKind: "terminated" }), ASTRA).unavailable, false);
});

test("HIGH + tool failure does not become model-unavailable evidence", () => {
  assert.equal(classifySeniorModelFailure({ outcome: "completed", errorCategory: "tool_result_error" }, ASTRA).unavailable, false);
  for (const errorCategory of ["tool_error", "message_delivery_failed", "finalization_failed", "agent_failed"]) {
    const evidence = classifySeniorModelFailure(modelError({ errorCategory, upstreamRequestIdHash: "sha256:test" }), ASTRA);
    assert.equal(evidence.unavailable, false, `${errorCategory} must not STOP even with an upstream request id`);
  }
});

test("generic agent failure and wrong-model errors are not unavailable", () => {
  assert.equal(classifySeniorModelFailure(modelError({ errorCategory: "Error" }), ASTRA).unavailable, false);
  assert.equal(classifySeniorModelFailure({ resolvedRef: "openai/gpt-5.6-sol", outcome: "error", errorCategory: "provider_unavailable" }, ASTRA).unavailable, false);
});

test("LOW risk failure does not meet HIGH-only stop precondition", () => {
  const evidence = classifySeniorModelFailure(modelError({ errorCategory: "provider_unavailable" }), ASTRA);
...
test("verified owner normal and HIGH are allowed", () => {
  const normal = applyLocalRiskDecision({ classification: { riskLevel: "LOW" }, verifiedOwner: true, existingFailClosed: false });
  const high = applyLocalRiskDecision({ classification: { riskLevel: "HIGH" }, verifiedOwner: true, existingFailClosed: true });
...
test("stored binding cannot authorize restart/recovery or names", () => {
  ...
  assert.equal(recordTrustedOwner(store, { ...ctx, senderIsOwner: true }, { senderIsOwner: true }), true);
  assert.equal(isTrustedOwner(store, ctx, {}), false);
  assert.equal(isTrustedOwner(store, { ...ctx, senderId: "attacker", username: "synthetic-name" }, {}), false);
});
...
test("trusted owner binds transport independently of conversation channelId", () => {
  ...
  const store = path.join(dir, "owners.json");
  const liveCtx = { senderIsOwner: true, channelId: "conversation-fixture", messageProvider: "telegram", accountId: "default", senderId: "1000000001" };
  ...
  assert.equal(isTrustedOwner(store, liveCtx, { accountId: "default", senderId: "1000000001" }), true);
});
...
test("inbound authoritative owner fields form the same immutable binding used after restart", () => {
  ...
  const store = path.join(dir, "owners.json");
  const inboundCtx = { senderIsOwner: true, channel: "telegram", channelId: "telegram", accountId: "default", senderId: "1000000001" };
  const inboundEvent = { channel: "telegram", accountId: "default", senderId: "1000000001", senderIsOwner: true };
  assert.equal(recordTrustedOwner(store, inboundCtx, inboundEvent), true);
  assert.equal(isTrustedOwner(store, { ...inboundCtx, messageProvider: "telegram", channel: undefined }, {}), true);
});

test("non-owner + genuine emergency + stopped remains fail-closed", () => {
  const result = applyLocalRiskDecision({ classification: { riskLevel: "HIGH" }, verifiedOwner: false, existingFailClosed: true });
