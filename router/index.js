...
import { clearStopState, classifySeniorModelFailure, isEmergencyHighRisk, readStopState, sanitizeTaskId, updateMetrics, writeStopState } from "./pilot.js";
...
function splitModel(ref) {
  ...
  if (index < 1 || index === ref.length - 1) throw new Error(`invalid canonical model ref: ${ref}`);
...
export default definePluginEntry({
  ...
  register(api) {
    ...
    const testSessionPrefixes = Array.isArray(config.testSessionPrefixes) ? config.testSessionPrefixes.filter(Boolean) : [];
    ...
    const inScope = (ctx) => {
      if (!ctx?.agentId || !agentIds.has(ctx.agentId)) return false;
      if (mode === "live") return true;
      return Boolean(ctx.sessionKey && testSessionPrefixes.some((prefix) => ctx.sessionKey.startsWith(prefix)));
    };
    const keyFor = (ctx, event) => event?.runId || ctx?.runId || (ctx?.sessionKey ? `session:${ctx.sessionKey}` : undefined);
    ...
    const takeBlocked = (ctx, event) => {
      ...
      if (blocked) {
        for (const key of keys) blockedTurns.delete(key);
      }
      return blocked;
    };
    ...
    api.on("before_agent_reply", (event, ctx) => {
      ...
      if (isEmergencyHighRisk(event.cleanedBody) && !isVerifiedOwner(ctx, event)) {
        ...
        if (reason) failClosed(ctx, event, reason);
      }
    ...
    api.on("before_model_resolve", (event, ctx) => {
      ...
      if (verifiedOwner) recordTrustedOwner(ownerStorePath, ctx, event);
      let classification;
      try {
        ...
        if (isEmergencyHighRisk(event.prompt) && !verifiedOwner) {
          return failClosed(ctx, event, stopped.stopped ? "router_stopped_emergency_high_risk" : "owner_authority_required");
        }
        if (key && decisions.has(key)) {
          writeStopState(statePath, "routing_loop_or_duplicate_invocation", { task_session_id: taskId });
      ...
      if (classification.target !== "ASTRA") return;
    ...
    api.on("model_call_ended", (event, ctx) => {
      ...
      if (!decision || decision.decision !== "ASTRA_OVERRIDE" || decision.risk_level !== "HIGH") return;
    ...
    api.on("llm_output", (event, ctx) => {
      ...
      if (verification.fallback) {
        ...
        if (stopState.stopped && stopState.trigger === "silent_fallback" && stopState.task_session_id === decision.task_session_id) {
          clearStopState(statePath, { trigger: "silent_fallback_recovered", task_session_id: decision.task_session_id });
        }
      }
    ...
    api.on("before_agent_finalize", (event, ctx) => {
      const decision = findDecision(ctx, event);
      if (!decision || !event.provider || !event.model) return;
    ...
    api.on("agent_end", (event, ctx) => {
      ...
      if (decision.result === "SELECTED") decision.result = event.success ? "UNVERIFIED" : "FAIL";
      if (!event.success) decision.result = "FAIL";
      appendAudit(logPath, auditRecord(decision));
      updateMetrics(metricsPath, decision);
      const key = keyFor(ctx, event);
      if (key) decisions.delete(key);
