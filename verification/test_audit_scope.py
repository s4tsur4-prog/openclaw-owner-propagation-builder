    ...
    def test_only_changed_explicit_operands(self):
        ...
        for command in receipt['commands']:
            if command['name'].startswith('affected-lint'):
                ...
                self.assertTrue(all('..' not in Path(p).parts and not Path(p).is_absolute() for p in operands))
                qualified = {'router/' + p if command['cwd'] == 'router' else p for p in operands}
                self.assertLessEqual(qualified, expected)
                targets.update(qualified)
        self.assertEqual(targets, expected)

    def test_router_escape_directory_and_option_rejected(self):
        for operand in ['.', 'test/', '../source/src/changed.ts', str(self.source / 'src/changed.ts'), '--help']:
            with self.subTest(operand=operand):
                self.put('router/patched-file-hashes.json', json.dumps({operand: 'after'}))
                self.assertNotEqual(self.run_audit().returncode, 0)

    def test_router_symlink_escape_rejected(self):
        (self.root / 'router/changed.mjs').unlink()
        (self.root / 'router/changed.mjs').symlink_to(self.source / 'src/changed.ts')
        self.assertNotEqual(self.run_audit().returncode, 0)

    def test_router_format_cwd_and_operands(self):
        self.assertEqual(self.run_audit().returncode, 0)
        receipt = json.loads((self.root / 'owner-receipts/scope-audit.json').read_text())
        for name in ['changed-format-router', 'affected-lint-router']:
            command = next(c for c in receipt['commands'] if c['name'] == name)
            self.assertEqual(command['cwd'], 'router')
            self.assertEqual(command['argv'][command['argv'].index('--') + 1:], ['changed.mjs'])
