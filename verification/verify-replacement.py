...
CANDIDATE = "550ae959b597b874ac4ce45fb220a8f35ddb6620"
...
def integrity(label):
    ...
    assert data["applied_diff_sha256"] == "6594510c8654539a347c28e5e2b80222dbf945f0362ca0c3bec5d25f997bfb67"
...
def execute_stage():
    if STAGE == "prepare":
        ...
        run("router-suite", ["node", "--test", "--test-reporter=tap", *[str(p) for p in sorted((ROOT / "router/test").glob("*.test.js"))]])
    elif STAGE == "targeted":
        tests("targeted-core", [
        ...
        for item in scope["commands"]:
            ...
            assert run(item["name"], item["argv"], cwd=ROOT / item["cwd"]) == 0
